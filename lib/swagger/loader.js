...
class Component {
  ...
  constructor(options) {
    ...
    component.getNames = (split => [split])
    component.children = []
    return component
  }
   ...
   */
  _getStream(options) {
    return this._request('GET', Object.assign({ stream: true }, options))
  }
   ...
   */
  _patch(options) {
    return this._request('PATCH', merge({
   ...
   */
  _post(options) {
    return this._request('POST', merge({
   ...
   */
  _put(options) {
    return this._request('PUT', merge({
      headers: { 'content-type': 'application/json' }
    }, options))
  }
}
